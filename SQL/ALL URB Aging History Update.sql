...
DECLARE @lastDayMonth AS DATE;

SET @lastDayMonth = DATEADD(day,-1,CAST(CAST(YEAR(CURRENT_TIMESTAMP) AS VARCHAR) + '-' + CAST(MONTH(CURRENT_TIMESTAMP) AS VARCHAR) + '-01' AS DATE));
...
WHERE Dates.date_id = @lastDayMonth
